  ...
  return (
    <label className="flex flex-col gap-2">
      ...
      <input
        ...
        className={`h-12 rounded-2xl bg-white/7 px-4 text-base text-paper
          border border-transparent placeholder:text-white/22
          focus:outline-none focus:border-[var(--accent)]
          aria-[invalid=true]:border-red-400/60 ${className}`}
        {...props}
      />
      {hint && !error && <span className="text-xs leading-relaxed text-white/35">{hint}</span>}
