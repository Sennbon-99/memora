...
function currentRollId(req: Parameters<RequestHandler>[0]): string {
...
export const join: RequestHandler = async (req, res, next) => {
  try {
    ...
    const session = await guestService.joinEvent(req.params.slug!, existing);
...
export const consent: RequestHandler = async (req, res, next) => {
  try {
    res.status(200).json(await guestService.giveConsent(currentRollId(req)));
  } catch (err) {
    next(err);
  }
};
...
export const identity: RequestHandler = async (req, res, next) => {
  try {
    ...
    res.status(200).json(await guestService.setIdentity(currentRollId(req), input));
  } catch (err) {
    next(err);
  }
};
...
export const saveCode: RequestHandler = async (req, res, next) => {
  try {
    ...
    res.status(200).json(await guestService.saveRecoveryCode(currentRollId(req), code));
  } catch (err) {
    next(err);
  }
};
...
export const recover: RequestHandler = async (req, res, next) => {
  try {
    ...
    const { deviceToken, rollId } = await guestService.recoverRoll(req.params.slug!, input);
