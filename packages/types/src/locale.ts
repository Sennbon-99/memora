...
const frenchErrorMap: ZodErrorMap = (issue, ctx) => {
  switch (issue.code) {
    ...
    case z.ZodIssueCode.invalid_string:
      if (issue.validation === 'email') return { message: 'Adresse électronique invalide' };
    ...
    case z.ZodIssueCode.too_small:
      if (issue.type === 'string') {
        return issue.minimum === 1
          ...
          : { message: `${issue.minimum} caractères au moins` };
      }
      if (issue.type === 'number') return { message: `Le minimum est ${issue.minimum}` };
      if (issue.type === 'array') return { message: `${issue.minimum} élément(s) au moins` };
    ...
    case z.ZodIssueCode.too_big:
      ...
      if (issue.type === 'array') return { message: `${issue.maximum} élément(s) au plus` };
      return { message: 'Valeur trop grande' };

    case z.ZodIssueCode.invalid_enum_value:
      return { message: 'Choisissez une des valeurs proposées' };
    ...
    case z.ZodIssueCode.not_multiple_of:
      return { message: `Doit être un multiple de ${issue.multipleOf}` };

    case z.ZodIssueCode.unrecognized_keys:
      return { message: 'Champ inconnu' };

    case z.ZodIssueCode.invalid_literal:
      return { message: 'Valeur attendue différente' };
